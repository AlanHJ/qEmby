...
AdminUserCard::AdminUserCard(UserInfo userInfo,
                             QString currentUserId,
                             QWidget* parent)
    : QFrame(parent)
    , m_user(userInfo)
    , m_currentUserId(currentUserId)
    , m_displayName(displayNameForUser(m_user)) {
    ...
    avatarLabel->setText(m_displayName.isEmpty()
                             ? QStringLiteral("?")
                             : m_displayName.left(1).toUpper());
    ...
    auto* lastLoginLabel = new QLabel(
        pageUsersText("Last login: %1")
            .arg(lastLoginText.isEmpty() ? QStringLiteral("-") : lastLoginText),
        this);
    ...
    auto* lastActivityLabel = new QLabel(
        pageUsersText("Last active: %1")
            .arg(lastActivityText.isEmpty() ? QStringLiteral("-") : lastActivityText),
        this);
    ...
    switchWidget->setToolTip(
        m_user.isDisabled ? pageUsersText("Enable user")
                          : pageUsersText("Disable user"));
    ...
    connect(switchWidget, &ModernSwitch::toggled, this,
            [this, switchWidget](bool checked) {
                switchWidget->setToolTip(checked
                                             ? pageUsersText("Disable user")
                                             : pageUsersText("Enable user"));
    ...
    connect(deleteBtn, &QPushButton::clicked, this, [this]() {
        emit deleteRequested(m_user.id, m_displayName);
    });
